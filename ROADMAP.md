...
**Progress:** the Pages viewer runs built-in sheet/T-shirt/cape/skirt/dress/poncho templates and normalized OBJ/self-contained GLB assets through a live Rust/WASM session. The built-in garment selector can place cloth against Rust-owned mannequin collider fixtures while the canvas only renders their descriptors. Template descriptions and inspection expose whether a mannequin is active without moving any simulation authority into JavaScript. It supports mesh-density changes, textile presets, gravity/solver-iteration/velocity-damping controls, pause/play/single-step/reset, ordinary vertex dragging, persistent movable pins, editable none/sphere/capsule collision scenery, renderer-only orbit/pan/zoom camera controls, and deterministic replay coverage. The inspector exposes normalized geometry fingerprints, constraint topology/error, rigid-contact/friction counts, and aggregate self-collision candidate/test/projection counts from Rust. Surface, wireframe, constraint, sampled-normal, and mannequin views remain renderer-derived consumers of authoritative positions/topology/collider descriptors. Generated reference snapshots remain a fail-closed fallback/evidence path. Remaining interactive work is primarily opt-in self-collision contact-location visualization, richer attachment editing, explicit seam/pattern modeling, and eventually the planned wgpu viewer when it provides concrete value over the current canvas consumer.

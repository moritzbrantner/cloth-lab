...
**Progress:** the import foundation now has a parser-independent `GarmentAsset`, deterministic simulation-geometry fingerprints, fail-closed OBJ and self-contained GLB geometry adapters, and a `TriangleMeshCloth` path for arbitrary indexed surfaces. Built-in deterministic sheet, T-shirt, cape, skirt, dress, and poncho drape templates provide repeatable garment experiments without pretending that single-surface fixtures contain sewing or pattern semantics. Mannequin-backed templates now bind selected cloth particles to semantic humanoid joints and derive moving collision primitives from the pinned `three-d-animation` hierarchy/keyframe contract in 3d-lab. Cloth Lab owns only the cloth-facing adaptation (collision shapes and attachment targets); animation sampling, quaternion interpolation, and parent/child transform evaluation remain in 3d-lab. Rest/Walk/Wave fixtures have deterministic animated cloth/contact replay coverage. Structural constraints come only from real mesh edges and reuse the existing mesh-general bending/collision/contact solver primitives. Plain mesh formats intentionally do not manufacture pattern-space shear, seam, or fabric semantics they do not contain.
...
**Progress:** the Pages viewer runs built-in sheet/T-shirt/cape/skirt/dress/poncho templates and normalized OBJ/self-contained GLB assets through a live Rust/WASM session. Mannequin-backed templates expose Rest/Walk/Wave motion and exact playback speed while the browser remains a consumer of Rust-owned cloth positions and animated collider descriptors. The sampled pose advances on the same explicit fixed timestep as cloth; semantic garment attachments follow sampled joints unless a user explicitly moves or removes that pin, which detaches it from mannequin control. Template descriptions and inspection expose mannequin joint count, active motion, clip time, and collider count without moving simulation or animation authority into JavaScript. It supports mesh-density changes, textile presets, gravity/solver-iteration/velocity-damping controls, pause/play/single-step/reset, ordinary vertex dragging, persistent movable pins, editable none/sphere/capsule collision scenery, renderer-only orbit/pan/zoom camera controls, and deterministic replay coverage. The inspector exposes normalized geometry fingerprints, constraint topology/error, rigid-contact/friction counts, and aggregate self-collision candidate/test/projection counts from Rust. Surface, wireframe, constraint, sampled-normal, and mannequin views remain renderer-derived consumers of authoritative positions/topology/collider descriptors. Generated reference snapshots remain a fail-closed fallback/evidence path. Remaining interactive work is primarily opt-in self-collision contact-location visualization, richer attachment authoring/import, explicit seam/pattern modeling, imported skeletal/animation adapters where source semantics are preserved, and eventually the planned wgpu viewer when it provides concrete value over the current canvas consumer.
